...
export const obtenirToutesCommandes = createAsyncThunk(
  ...
  async ({ userId, page = 0, taille = 10 }, { rejectWithValue }) => {
    try {
      ...
      const reponse = await CommandeService.obtenirToutesCommandes(
        userId,
        page,
        taille
      );
      ...
      return rejectWithValue(
        erreur.response?.data || "Erreur lors de la récupération des commandes."
      );
    }
  }
);
...
export const mettreAJourCommande = createAsyncThunk(
  ...
  async ({ id, donneesMiseAJour }, { rejectWithValue }) => {
    try {
      const reponse = await CommandeService.mettreAJourCommande(
        id,
        donneesMiseAJour
      );
      return reponse;
...
export const updateStatutCommande = createAsyncThunk(
  "commandes/updateStatut",
  async ({ id, nouveauStatut }, { rejectWithValue }) => {
    try {
      const response = await CommandeService.updateStatutCommande(
        id,
        nouveauStatut
      );
      return response;
...
export const obtenirCommandesPayeesEtEnCours = createAsyncThunk(
    "commandes/obtenirCommandesPayeesEtEnCours",
    async (_, { rejectWithValue }) => {
        try {
            const reponse = await CommandeService.obtenirCommandesPayeesEtEnCours();
            return reponse;
        } catch (erreur) {
            return rejectWithValue(erreur.response?.data || "Erreur lors de la récupération des commandes payées et en cours.");
        }
    }
);
...
const commandeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(obtenirCommandeParId.pending, (state) => {
        state.chargement = true;
        state.erreur = null;
      })
      .addCase(obtenirCommandeParId.fulfilled, (state, action) => {
        console.log(
          "Commande récupérée via Redux obtenir commande par id fulfilled :",
          action.payload
        );
        state.chargement = false;
        state.commandeActuelle = action.payload;
      ...
      .addCase(obtenirCommandeParId.rejected, (state, action) => {
        state.chargement = false;
        state.erreur = action.payload;
      })
      ...
      .addCase(obtenirToutesCommandes.rejected, (state, action) => {
        state.chargement = false;
        state.erreur =
          action.payload || "Erreur lors de la récupération des commandes.";
        console.error(
      ...
      .addCase(updateStatutCommande.pending, (state) => {
        state.chargement = true;
        state.erreur = null;
      })
      .addCase(updateStatutCommande.fulfilled, (state, action) => {
        ...
        const index = state.commandes.findIndex(
          (cmd) => cmd.id === action.payload.id
        );
        if (index !== -1) {
          state.commandes[index].statut = action.payload.statut;
        }
      })
      .addCase(updateStatutCommande.rejected, (state, action) => {
        state.chargement = false;
        state.erreur = action.payload;
      })
    .addCase(obtenirCommandesPayeesEtEnCours.pending, (state) => {
        state.chargement = true;
        state.erreur = null;
    })
    .addCase(obtenirCommandesPayeesEtEnCours.fulfilled, (state, action) => {
        state.chargement = false;
        console.log("Payload reçu dans Redux :", action.payload);
        state.commandes = action.payload;
    })
    .addCase(obtenirCommandesPayeesEtEnCours.rejected, (state, action) => {
        state.chargement = false;
        state.erreur = action.payload;
    })
      .addMatcher(
